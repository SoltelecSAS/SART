 ...
 * @author Gerencia TIC
 */
public class FrenosPrueba implements PruebaDefault {
...
//            }
    ...
    private void calcularEficacia() {
        ...
        sumaPesos = calcularSumaPesos();
        sumaFuerzas = calcularSumaFuerzas();

        if (fuerzasInconsistentes(sumaFuerzas)) {
            return;
        }

        sumFreAux = calcularSumaFuerzasAux();

        if (fuerzasAuxiliaresInconsistentes(sumFreAux)) {
            return;
        }
    ...
    private double calcularSumaPesos() {
        double suma = 0;
        for (int i = 0; i < pesoIzquierdo.size(); i++) {
            suma += pesoDerecho.get(i) + pesoIzquierdo.get(i);
        }
        System.out.println("Sumatoria Pesos para calculo " + suma);
        return suma;
    }

    private double calcularSumaFuerzas() {
        double suma = 0;
        for (int i = 0; i < fuerzaDerecha.size(); i++) {
            if (!fuerzaDerecha.isEmpty() && fuerzaDerecha.get(0) > 0) {
                suma += fuerzaDerecha.get(i) + fuerzaIzquierda.get(i);
            }
            System.out.println("Sumatoria Fuerzas para calculo " + suma);
        }
        return suma;
    }
    ...
    private double calcularSumaFuerzasAux() {
        double suma = 0;
        for (int i = 0; i < fuerzaDerechaAux.size(); i++) {
            suma += fuerzaDerechaAux.get(i) + fuerzaIzquierdaAux.get(i);
            System.out.println(" *****************----------");
            System.out.println("fuerz Aux. DEr " + fuerzaDerechaAux.get(i) + " FuerzaAux. Izq: " + fuerzaIzquierdaAux.get(i));
        }
        return suma;
